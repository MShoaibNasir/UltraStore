	...
	<div class="col-12">
		...
		<div class="card">
			<div class="card-header">
				<span class="panel-title">{{ _lang('Shipping Report') }}</span>
			</div>
			
			<div class="card-body">
			
				<div class="report-params">
					<form class="validate" method="post" action="{{ route('reports.shipping_report') }}">
						<div class="row">
              				{{ csrf_field() }}
				...
				<table class="table table-bordered report-table">
					...
					@if(isset($report_data))
						@foreach($report_data as $report) 
							<tr>
								<td>{{ date($date_format, strtotime($report->start_date)).' - '.date($date_format, strtotime($report->end_date)) }}</td>
								<td>{{ $report->shipping_method }}</td>
