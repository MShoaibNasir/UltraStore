(function($) {
	...
	$(document).on("click",".ajax-modal-2",function(){
	 ...
	 //Media Modal
	 var current_media, multiple = false;
	 $(document).on("click",".choose-media",function(){
		 var link = _url +'/media';

		 current_media = $(this);
		 var title = $lang_media_list;
         
		 if($(this).data('multiple') == true){
		 	link = link + '?multiple=true';
		 	multiple = true;
		 }
	...
	$( document ).on( "select-media", function( event, media_id, media_path ) {
	    var field_name = $(current_media).data('input-name');
	    $(current_media).next('.image-container').html('');
	    $(current_media).next('.image-container').append(`<div class="image">
														    	<img src="${media_path}">
														    	<input type="hidden" name="${field_name}" value="${media_id}">
	...
	$( document ).on( "select-multiple-media", function( event, media_id, media_path ) {
	    var field_name = $(current_media).data('input-name');
	    $(current_media).next('.multiple-image-container').append(`<div class="image">
														    	<img src="${media_path}">
														    	<input type="hidden" name="${field_name}" value="${media_id}">
														    	<i class="remove-media fas fa-times"></i>
														    </div>`);
	...
	//Tax rates
	$(document).on('change','#tax-rates-table .country > select',function(){
		var elem = $(this);
		var country = $(elem).val();

		if(country != ""){
			var country_id = $(this).find(':selected').data('id');
			$.get(_url + '/taxes/get_states/' + country_id, function(data, status){
		    	var json = JSON.parse(data);
		    	$(elem).parent().next().find('select').html('<option value="*">*</option>');
		    	$.each(json, function(i, item) {
				    $(elem).parent().next().find('select').append(`<option value="${item.name}">${item.name}</option>`);
				});
		  	});
		}else{
			$(elem).parent().next().find('select').html('<option value="*">*</option>');
	...
	$(document).on('click','#add_new_tax_rate',function(){
        var row = $("#tax-rates-table tbody tr:eq(0)").clone();
        $(row).find('.tax_name').find('input').val("");
        $(row).find('.country').find('select').val("");
        $(row).find('.state').find('select').html("");
        $(row).find('.rate').find('input').val("");
        $("#tax-rates-table tbody").append(row);
	});

	$(document).on('click','#tax-rates-table .remove-row',function(){
        if($("#tax-rates-table tbody tr").length > 1){
        	$(this).parent().parent().remove();
        }
	});

	$(document).on("change","#order-update-form #payment_status",function(){
		$(this).val() == "completed" ? $("#order-update-form .transaction_id").removeClass('d-none') : $("#order-update-form .transaction_id").addClass('d-none');
	});

	/*=====================================
  	Select2 select products
	======================================*/ 
	$('.select-products').select2({
	  minimumInputLength: 2,
	  ajax: {
	    url: function (params) {
	      return _asset_url + 'shop/search_products';
	    },
	    processResults: function (data) {
...
function init_datepicker(){
    ...
    if ($(".clear-datepicker").length) {
		$('.datepicker').each(function(i, obj) {
	    	$(this).next('.fake_datepicker').html("");
	    })
    }
